...
void UpdateBullet(void)
{
	...
	for (nCntbullet = 0; nCntbullet < MAX_BULLET; nCntbullet++)
	{
		...
		{
			SetEffect(g_aBullet[nCntbullet].pos, D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXCOLOR(0.6f, 0.2f, 0.6f, 1.0f), 60.0f, 1.0f, 0.01f, 2.0f, 0.0f, 30);
			...
			if (g_aBullet[nCntbullet].type == BULLETTYPE_PLAYER)
			{
				...
				//fRotMove = atan2f(g_aBullet[nCntbullet].move.x, g_aBullet[nCntbullet].move.y);  //現在の移動量

				//fRotDest = atan2f(pEnemy->pos.x- g_aBullet[nCntbullet].pos.x, pEnemy->pos.y - g_aBullet[nCntbullet].pos.y);   //目標の移動量

				//fRotDiff = fRotDest - fRotMove;   //目標の移動方向への差分

				////角度の修正
				//if (fRotDest == -3.00f && fRotDest == 3.00f)
				//{
				//	fRotDest += 6.28f;
				//}

				//fRotMove += fRotDiff * 0.03f;  //移動方向への補正
			...
			if (g_aBullet[nCntbullet].nLife <= 0)
			{
				...
				SetPartycle(g_aBullet[nCntbullet].pos);
